...
Uses direction vector that removes weight from a "bad" away vertex (v_k) to a 
"good" FW vertex (s_k) with a gamma_max no more than the weight already in v_k.

    d_k^PFW = s_k - v_k ,        gamma_max = alpha_{v_k}.
...
The active set S^(k) is held as a dict  {(i, sign): alpha_v}  with the invariant
    sum_v alpha_v = 1,   alpha_v >= 0,
and the iterate is reconstructed from the weights at every step,
    x_k = sum_{v in S^(k)} alpha_v * v,
rather than carried as a separately-drifting incremental variable 
(x_k+1 = x_k + gamma*d_k) that could compound floating-point errors in each iteration.
...
def _reconstruct_x(weights, tau, n):
    """ Reconstructs x_k based on convex combination of weights in the active set:
    x = sum_{(i,sign) in S} alpha * sign * tau * e_i (source of truth for x_k)."""
...
def pairwise_fw(A, b, tau, x0=None, epsilon=1e-6, max_iter=10_000):
    ...
    2. Away Atom (Active Set):
       v_k := argmax_{v in S^(k)} <grad f(x_k), v> 
    3. Pairwise Direction & Step Bounds:
       d_k := s_k - v_k
       gamma_max := alpha_{v_k}
    4. Exact Line Search:
       gamma_k := argmin_{gamma in [0, gamma_max]} f(x_k + gamma * d_k)
    5. Pairwise Mass Transfer (Weight Update):
       alpha_{v_k} := alpha_{v_k} - gamma_k
       alpha_{s_k} := alpha_{s_k} + gamma_k
       (Swap step: If gamma_k == gamma_max, the away atom v_k is fully emptied 
       and dropped from the active set S^(k)).
    6. Stopping Criterion:
       The algorithm evaluates the standard FW gap: g_k := <grad f(x_k), x_k - s_k>.
       Terminate if g_k <= epsilon.
    ...
    for k in range(max_iter):
        ...
        if not stop:
            ...
            # Reconstruct x_k+1 from the weights (source of truth).
            x = _reconstruct_x(weights, tau, n)
